...
// NEW: Import the Language Switcher
  ...
  return (
    <div className="w-full px-4 pt-4 pb-2 sticky top-0 z-[100]">
      ...
      >
        ...
        {/* Desktop Navigation - Added flex-1, justify-center, responsive text sizing, and tighter gaps */}
        <nav className="hidden lg:flex flex-1 justify-center items-center gap-4 lg:gap-5 xl:gap-8 font-medium px-4 text-sm xl:text-base">
          {menuItems.map((item, index) => (
            <div key={index} className="relative group">
              <Link href={getLocalizedUrl(item.url)} className="hover:underline transition-colors py-2 flex items-center gap-1 text-center">
                {item.name}
              ...
              {item.subItems && item.subItems.length > 0 && (
                <div className="absolute top-full left-0 mt-4 w-56 bg-white text-[#135D66] rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible group-hover:mt-2 transition-all duration-300 ease-out border border-gray-100 overflow-hidden text-left text-base">
                  {item.subItems.map((sub, sIndex) => (
                    <Link key={sIndex} href={getLocalizedUrl(sub.url)} className="block px-5 py-3 hover:bg-gray-50 hover:underline hover:pl-7 transition-all duration-200 border-b border-gray-50 last:border-0 font-medium">
                      {sub.name}
                    </Link>
                  ))}
                </div>
        ...
        {/* Right Side Icons & Mobile Hamburger - Added shrink-0 to protect spacing */}
        <div className="flex items-center gap-3 sm:gap-6 shrink-0 z-10">
          
          {/* NEW: Inserted Language Switcher */}
          <LanguageSwitcher />
