    ...
    <div className="w-full px-4 pt-4 pb-2 sticky top-0 z-[100]">
      ...
      >
        ...
        <nav className="hidden lg:flex items-center gap-8 xl:gap-12 font-medium">
          {menuItems.map((item, index) => (
            <div key={index} className="relative group">
              <Link href={getLocalizedUrl(item.url)} className="hover:underline transition-colors py-2 flex items-center gap-1">
                {item.name}
                {item.subItems && item.subItems.length > 0 && (
                  <svg className="w-4 h-4 transform group-hover:rotate-180 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
        ...
        <div className="flex items-center gap-3 sm:gap-6">
          ...
          <button className="hidden sm:block hover:text-[#98D80D] transition-colors hover:scale-110 transform duration-200">
            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
            </svg>
          </button>
